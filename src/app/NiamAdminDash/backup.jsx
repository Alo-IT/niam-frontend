...
export default function habijabi() {
  const onChange = (e) => console.log(`radio checked:${e.target.value}`);
  const {
    signedIn,
    isAuthenticated,
  ...
  useEffect(() => {
    setSelectedOrg(orgsArray.length > 0 ? orgsArray[0].value : "");
  }, [orgsArray]);
  ...
  return (
    <>
      ...
      <div className="orgDetails">
        <p>Org Domain: {orgInfo?.data[0].orgDomain}</p>
        <p>Org Name: {orgInfo?.data[0].organizationName}</p>
        <p>Org User Tyre: {orgInfo?.data[0].userTyre}</p>
      </div>
      <div
        className="logoutButton"
        align="middle"
